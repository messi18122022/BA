...
ORDNER_MIT_SÄULE = "manuels_data/mit"
ORDNER_OHNE_SÄULE = "manuels_data/ohne"

# Ordner, in dem die Plots gespeichert werden sollen
OUTPUT_DIR = "manuels_data"  # Dieser Ordner wird sowohl für den van Deemter Plot als auch für die H-Werte-Rechnung genutzt

# Standard-Flussraten-Vektor (Standard: 2.5, 2.4, 2.3, ..., 0.1)
default_flow_rates = np.array([0.1,0.2,0.3,0.05,0.6,0.07,0.8,0.9,0.15,0.25])
# ----------------------------------------------------------
...
def process_files(filepaths, beschriftung):
    ...
    for datei in filepaths:
        try:
            try:
                df = pd.read_csv(datei, delimiter='\t', encoding='utf-16', header=None)
            except Exception:
                df = pd.read_csv(datei, delimiter=',')  # Fallback für alte Formate
            df = df.iloc[:, :2]  # Nur die ersten zwei Spalten verwenden
            if df.shape[1] < 2:
...
def main():
    ...
    print(f"Gefundene Dateien (mit Säule): {files_with_column}")
    ...
    print(f"Gefundene Dateien (ohne Säule): {files_without_column}")
